...
export default Ember.Component.extend({
	...
	actions: {
		...
		showNextSection: function showNextSection(nextSecId) {
			var self = this;
			var userId = Ember.$("[elname=testSectionBtn]").attr("id");
			$("#questionSections input[type=radio]:checked").each(function (index, element) {
				var testId = Ember.$(element).attr("testid");
				var questionId = Ember.$(element).attr("name");
				var userOption = Ember.$(element).val();
